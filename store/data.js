...
export const actions = {
  ...
  async getBalances({ commit, state: { api } }, { address, currency }) {
    try {
      ...
      const returnLunaBalance = []
      balances.forEach(function (item) {
        if (item.id === 'LUNA') {
          returnLunaBalance.push(item)
        }
      })
      commit('setBalances', returnLunaBalance)
  ...
  async getLunaApr({ commit, state: { api } }) {
    try {
      const apr = await api.getLunaApr()
      commit('setLunaApr', Number(apr.data.cmc_supply_apr[0].apr).toFixed(1))
      commit('setLunaAprLoaded', true)
    } catch (err) {
      commit(
        'notifications/add',
        {
  ...
  async getLunaValue({ commit, state: { api } }) {
    try {
      const lunaValue = await api.getLunaValue()
      console.log(lunaValue)
      commit('setLunaValue', lunaValue.data['terra-luna-2'].usd)
      commit('setLunaValueLoaded', true)
    } catch (err) {
      commit(
        'notifications/add',
        {
          type: 'danger',
          message: 'Getting Convert Luna failed:' + err.message,
        },
        { root: true }
      )
    }

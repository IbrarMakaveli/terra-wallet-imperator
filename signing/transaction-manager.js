    ...
    switch (messageType) {
      ...
      case 'StakeTx':
        try {
          ...
          const getTx = await delegateTokensLuna(
            signer,
            senderAddress,
            message.to[0],
            finalAmout,
      ...
      case 'UnstakeTx':
        try {
          const finalAmout = (message.amount.amount * 1000000).toString()
          const getTx = await unDelegateTokensLuna(
            signer,
            message.from[0],
            senderAddress,
            finalAmout,
      ...
      case 'RestakeTx':
        try {
          const finalAmout = (message.amount.amount * 1000000).toString()
          const getTx = await ReDelegateTokensLuna(
            signer,
            message.delegator[0],
            message.from[0],
            message.to[0],
...
async function rewardLuna(sign, addFrom, addTo, fee, signingType) {
  let wallet = ''
  if (signingType === 'local') {
    wallet = await DirectSecp256k1HdWallet.fromMnemonic(sign.secret.data, {
      prefix: network.addressPrefix,
...
async function delegateTokensLuna(
  sign,
  addFrom,
  addTo,
  amountLuna,
...
async function unDelegateTokensLuna(
  sign,
  validator,
  fromDel,
  amountLuna,
...
async function ReDelegateTokensLuna(
  sign,
  delegator,
  valFrom,
  valTo,
...
async function voteTxLuna(sign, fromDel, proposalIdd, vote, fee, signingType) {
  let wallet = ''
  if (signingType === 'local') {
    wallet = await DirectSecp256k1HdWallet.fromMnemonic(sign.secret.data, {
      prefix: network.addressPrefix,
